...
export function Contact({ data }: { data: ContactData }) {
  ...
  const contactInfo = [
    { icon: Mail, labelKey: "contact.info.email", value: data.email },
    { icon: Phone, labelKey: "contact.info.phone", value: data.phone },
    { icon: MapPin, labelKey: "contact.info.location", value: language === "ar" ? data.ar_location : data.en_location },
    { icon: Clock, labelKey: "contact.info.hours", value: language === "ar" ? data.ar_hours : data.en_hours },
  ]
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-5 gap-12">
          ...
          >
            ...
            <h2 className="text-3xl sm:text-4xl font-bold text-[#07223F] mb-4">{t("contact.title")}</h2>
            ...
            <div className="space-y-4">
              {contactInfo.map((item, index) => (
                ...
                >
                  ...
                  <div>
                    <p className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">{t(item.labelKey)}</p>
                    <p className="font-semibold text-[#07223F]" dir={item.labelKey === "contact.info.email" ? "ltr" : undefined}>{item.value}</p>
                  </div>
                </div>
              ))}
            </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <label htmlFor="name" className="text-xs font-semibold text-muted-foreground uppercase tracking-wider block">
                        {t("contact.form.name")}
                      </label>
                      <input
                        type="text"
                  ...
                  <div className="space-y-2">
                    <label htmlFor="service" className="text-xs font-semibold text-muted-foreground uppercase tracking-wider block">
                  ...
                  <div className="space-y-2">
                    <label htmlFor="message" className="text-xs font-semibold text-muted-foreground uppercase tracking-wider block">
