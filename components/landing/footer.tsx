...
import { Linkedin, Twitter, Instagram, Facebook, ArrowUp, LucideIcon } from "lucide-react"
import { useEffect, useState } from "react"
import { useLanguage } from "@/lib/language-context"

const iconMap: Record<string, LucideIcon> = { Linkedin, Twitter, Instagram, Facebook }
...
export function Footer({ socialLinks }: { socialLinks: SocialItem[] }) {
            ...
            <div>
              ...
              <ul className="space-y-3">
                {footerLinks.services.map((link) => (
                  <li key={link.label}>
                    <Link href={link.href} className="text-white/60 hover:text-[#1877F2] text-sm transition-colors">
                      {link.label}
                    </Link>
                  </li>
                ))}
